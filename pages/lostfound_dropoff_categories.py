...
class FoundDropoffForm(tk.Canvas):
    def __init__(self, root, mode="dropoff", **kwargs):
    ...
    def submit_form(self):
        ...
        if self.mode == "dropoff":
            ...
            if self.match_item_info():
                messagebox.showinfo("Success", "The item will be retrieved.")
                self.root.show_retrieve_compartment_page()
            else:
                messagebox.showerror("Error", "Item details do not match any compartment.")

    def match_item_info(self):
        # Add logic here to check if the provided details match the stored compartment info
        return True
